...
This project implements Zero-Reference Deep Curve Estimation (Zero-DCE) for enhancing low-light images and videos without paired or reference data. A lightweight CNN estimates pixel-wise curves to improve illumination while preserving color and detail.
...
Use train_model.py to train Zero-DCE on a dataset of low-light images.
...
--IMG_H, --IMG_W, --IMG_C : Image height, width, channels
...
Example:
train_model.py --dataset_dir ./data --epoch 100 --batch_size 8
...
Example:
test_model.py --model_path ./checkpoints/model.pth --dataset_path ./test_data
...
--save_result : Save enhanced image

--iteration : Curve iteration count

Example:
single_image_enhance.py --model_path model.pth --image_path input.jpg --save_result True

Notes

The model does not require paired or reference images.

Uses non-reference losses such as exposure control, spatial consistency, color constancy, and illumination smoothness.

Supports both image and video enhancement workflows.
